...
def plot_mesh_attribute(mesh: mi.Mesh, attrib_name: str, is_color: bool = False):
    m = plot_mesh(mesh)
    ...
    if attrib_size == 1:
        m.add_scalar_quantity(attrib_label, values, defined_on=domain, enabled=True)
    elif attrib_size == 3:
        if is_color:
            m.add_color_quantity(attrib_label, values.reshape(-1,3), defined_on=domain, enabled=True)
        else:
            m.add_vector_quantity(attrib_label, values.reshape(-1,3), defined_on=domain, enabled=True)
    else:
        for i in range(attrib_size):
            m.add_scalar_quantity(attrib_label + f"{i}", values[i::attrib_size], defined_on=domain, enabled=True)
