...
def test_principled(num_samples = 1 << 20, random_inputs: bool = False):
    ...
    eval_pdf_ref = bsdf_ref.eval(ctx, si, wo)
    eval_pdf_impl = bsdf_impl.eval(si, wo, ALL_ACTIVE)

    # Evaluate error
    print("\t\tMaxNorm\t\tL2Norm")
    ...
    print(f"\ttype\t{dr.allclose(bs_ref.sampled_component, bs_impl.sampled_component)}" + \
